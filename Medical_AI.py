...
if str_button:
    ...
    if Inprompt == True:
        ...
        prompt_bot = PromptTemplate(
            template=template_bot, input_variables=["question"])
        llm_chain = LLMChain(prompt=prompt_bot, llm=HuggingFaceHub(
            repo_id="google/flan-t5-xxl", model_kwargs={"temperature": 0.01, "max_length": 64}), verbose=True)
        st.warning("Question: " + question)
        st.success("Baymax: " + llm_chain.run(question=question))
    else:
